...
| PR2-E2-S9c-2 | Borrar el archivo legacy `storage/tracking_repository.py`. | 4 type-hints en `monitors/*` (`SqliteTrackingRepository`→facade/Protocol) + **22 archivos de test** que construyen `SqliteTrackingRepository` (migrar al facade greenfield o borrar los redundantes con `tests/adapters/storage/*`) + borrar el archivo. | Ningún módulo NI test importa `storage.tracking_repository`; el archivo se borra; suite verde. | PR2-E2-S9c-1 | TODO — paso final grande: 22 test files usan el legacy. Es su propio pase (parte es borrar tests redundantes ya cubiertos por tests/adapters/storage/). |
