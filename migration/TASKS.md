...
| PR2-E2-S9 | Crear facade de storage y cortar imports legacy. | `adapters/storage/__init__.py`, composition root actual, tests de integración storage | Facade compone S1-S7 e implementa los ports necesarios; imports de runtime/services apuntan al facade nuevo; suite verde con DB greenfield; `storage/tracking_repository.py` queda sin uso y solo se borra cuando los tests confirmen paridad mínima. | PR2-E2-S1, PR2-E2-S2, PR2-E2-S3, PR2-E2-S4, PR2-E2-S5, PR2-E2-S6, PR2-E2-S7, PR2-E2-S8 | BLOCKED @claude (2026-07-02) — swap SOLO parcial: el composition root inyecta el facade a los **services**, pero ~40 sitios (`bot/alerts.py`, `bot/handlers.py`, `bot/jobs/*`, `monitors/change_detection.py`, `core/timezones.py`, tipos compartidos) siguen usando el **global legacy** `tracking_repository` (esquema viejo `active_events`/`tracked_competitions`). Runtime corre con DOS esquemas a la vez → incoherente. Borrar el legacy ahora rompe ~40 imports. NO estaba "casi listo". Reabierto como migración por capas (ver PR2-E2-S9a/b/c). |
| PR2-E2-S9a | Migrar `bot/alerts.py` + tipos compartidos al facade (sacar el global legacy de alerts). | `bot/alerts.py`, tipos de dominio | alerts no importa `tracking_repository` global; usa el repo inyectado o el facade; tests verdes. | PR2-E2-S9 | TODO |
| PR2-E2-S9b | Migrar `bot/jobs/*` + `core/timezones.py` al facade. | `bot/jobs/tasks.py`, `bot/jobs/legacy.py`, `core/timezones.py` | Jobs y timezones dejan de usar el global legacy; blocklist (`unified_merge_exceptions`) re-portado al esquema greenfield. | PR2-E2-S9a | TODO |
| PR2-E2-S9c | Migrar `bot/handlers.py` (×11 usos) + `monitors/change_detection.py` al facade; recién ahí borrar el legacy. | `bot/handlers.py`, `monitors/change_detection.py`, `storage/tracking_repository.py` | Ningún módulo importa `storage.tracking_repository`; se borra el archivo legacy; suite verde. | PR2-E2-S9b |
