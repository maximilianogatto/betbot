...
| PR1-T4 | Envolver lecturas/escrituras SQLite pesadas en `asyncio.to_thread`. | `storage/tracking_repository.py` (llamadas en `services/tracking.py`, `change_detection.py`) | Las rutas calientes (upsert de eventos, baselines, blobs) no bloquean el event loop. Tests verdes. | — | DONE |
...
| PR2-E4 | Mover servicios de `services/` a `services/` y adelgazarlos (sin telegram, sin SQL inline). | IN_PROGRESS @gemini (2026-07-22) |
