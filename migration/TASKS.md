...
| PR1-T4 | Envolver lecturas/escrituras SQLite pesadas en `asyncio.to_thread`. | `storage/tracking_repository.py` (llamadas en `monitors/tracking.py`, `change_detection.py`) | Las rutas calientes (upsert de eventos, baselines, blobs) no bloquean el event loop. Tests verdes. | — | IN_PROGRESS @gemini (2026-06-25) |
