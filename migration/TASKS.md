...
| PR1-T3 | Reinicio graceful de Chromium por RAM. | `bot/jobs/*` (resource monitor), `core/browser_handler.py`, `bot/config.py` | Cuando la RAM de Chromium supera el umbral, se llama `request_restart()` (espera `active_pages==0`, no kill). `ENABLE_MONITORING=true` por defecto. | — | IN_PROGRESS @gemini (2026-06-25) |
