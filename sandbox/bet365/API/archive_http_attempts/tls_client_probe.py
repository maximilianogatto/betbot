...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prueba replay HTTP con tls-client usando fingerprint Chrome sin Playwright.",
    )
    parser.add_argument("curl_file", help="Ruta a captured_curl.txt")
    parser.add_argument(
        "--out-dir",
        default=str(API_ROOT / "tls_client_attempts"),
        help="Directorio base donde guardar resultados.",
    )
    parser.add_argument(
        "--client-identifier",
        default="chrome_120",
        help="client_identifier de tls-client, por ejemplo chrome_120.",
    )
...
def build_sequences(markets_url: str) -> dict[str, list[str]]:
    ...
    return {
        ...
        "C_home_liga_endpoint": [home_url, competition_url, markets_url],
    }
    ...
    *,
    ...
    try:
        ...
        return {
            ...
            "error": f"tls_client unavailable: {type(error).__name__}: {error}",
        }
    ...
    host = urlsplit(request_data["url"]).hostname or "www.bet365.es"
    ...
    for index, url in enumerate(steps, start=1):
        ...
        requests_log.append(sanitize_request_payload(step_request))
        try:
        ...
        if index == len(steps):
            ...
            warmup_steps.append(step_summary)
            time.sleep(sleep_seconds)
...
def main() -> int:
    ...
    sequences = build_sequences(request_data["url"])
    ...
    write_json(run_dir / "table_rows.json", rows)
    print(json.dumps(rows, ensure_ascii=False, indent=2))

    useful_case = next((row for row in rows if row.get("useful")), None)
    if useful_case:
