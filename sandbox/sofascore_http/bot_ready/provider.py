"""Bot-ready SofaScore adapter backed by the HTTP-only research client.

This module intentionally lives under `sandbox/`. It implements BetBot's real
`StatsProvider` contract so discovery, fixture matching and report generation
can be validated before the provider is registered in production.
"""
...
class SofaScoreBotReadyStatsProvider(StatsProvider):
