"""Pure normalizers for Svenskfotboll HTTP and FOGIS XML payloads."""
...
class _TableHTMLParser(HTMLParser):
    ...
    def handle_endtag(self, tag: str) -> None:
        if tag in {"td", "th"} and self._cell_text is not None and self._row is not None:
            ...
            self._row.append(TableCell(text=text, links=tuple(self._cell_links)))
            self._cell_text = None
            self._cell_links = []
        elif tag == "tr" and self._row is not None:
            if any(cell.text for cell in self._row):
                self.rows.append(TableRow(cells=tuple(self._row)))
            self._row = None


def parse_widget_table(html: str) -> list[TableRow]:
    """Parse the HTML table embedded in Svenskfotboll widget JSON."""
...
def parse_competition_tree(payload: dict[str, Any]) -> list[dict[str, Any]]:
    """Flatten `/api/comp-find/filter` into unique competition options."""

    records: dict[str, dict[str, Any]] = {}
    groups = payload.get("competitions", []) if isinstance(payload, dict) else []
    for group in groups:
    ...
    return sorted(normalized, key=_competition_sort_key)
...
def search_competitions(
    ...
    """Filter flattened competitions by query and optional association id."""
...
def summarize_live_events(events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    red_cards = [
        event
        for event in events
        if "RED" in (event.get("type") or "").upper() or "red" in (event.get("type-desc") or "").casefold()
    ]
...
def extract_id_from_links(links: list[str], key: str) -> str | None:
    for link in links:
        values = parse_qs(urlparse(link).query).get(key)
        if values:
            return str(values[0])
    return None
...
def _parse_game_node(game_node: ET.Element) -> dict[str, Any]:
    ...
    return {
        ...
        "competition_id": game_node.attrib.get("competition-id")
        or (tournament_node.attrib.get("id") if tournament_node is not None else None),
...
def _competition_sort_key(item: dict[str, Any]) -> tuple[int, str]:
    name = str(item.get("name") or "").casefold()
    categories = " ".join(item.get("categories") or []).casefold()
    current_year = 2026
    is_current = f"{current_year}" in name
    senior = "senior" in categories or "allsvenskan" in categories or "superettan" in categories or "ettan" in categories
    return (0 if is_current and senior else 1 if is_current else 2, name)
