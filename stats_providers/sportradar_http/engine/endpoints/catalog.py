...
class EndpointSpec:
    ...
    Args:
        ...
        allow_refresh: When False, a blocked/expired payload from this endpoint
            never triggers a session bootstrap. Use for optional/live endpoints
            (e.g. `/common/` live feeds) so a best-effort miss does not open a
            browser to re-sign a token that is still valid.
...
ENDPOINT_SPECS: dict[str, EndpointSpec] = {
    ...
    "stats_match_situation": EndpointSpec(
        ...
        live=True,
        notes=(
            "Best-effort live feed on the /common/ namespace. For non-live matches it "
            "can return a blocked/expired-looking payload; allow_refresh=False keeps it "
            "from triggering a browser bootstrap when the cached token is still valid."
        ),
    ),
}
...
def call_endpoint(client: Any, name: str, **kwargs: Any) -> dict[str, Any]:
    ...
    return client.get_gismo(
        ...
        allow_refresh=spec.allow_refresh,
    )
...
def render_endpoint_catalog_v2() -> str:
    ...
    lines = [
        ...
        "",
        "This catalog is generated from typed endpoint specs in `stats_providers/sportradar_http/engine/endpoints/catalog.py`.",
        "",
