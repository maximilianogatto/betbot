...
class BotAlertsTests(unittest.TestCase):
    ...
    def test_split_telegram_message_counts_flags_as_utf16(self) -> None:
        # Each regional-indicator symbol occupies two UTF-16 code units.  This
        # reproduces the long, flag-heavy payload generated by /leagues.
        text = "\n".join([f"{index}. 🇦🇺 League" for index in range(40)])

        chunks = split_telegram_message(text, max_len=100)

        self.assertGreater(len(chunks), 1)
        self.assertEqual(
            "".join(chunk if index == 0 else "\n" + chunk for index, chunk in enumerate(chunks)),
            text,
        )
        self.assertTrue(all(len(chunk.encode("utf-16-le")) // 2 <= 100 for chunk in chunks))

    def test_split_telegram_message_splits_long_emoji_line_on_utf16_limit(self) -> None:
        chunks = split_telegram_message("🏴" * 20, max_len=10)

        self.assertEqual("".join(chunks), "🏴" * 20)
