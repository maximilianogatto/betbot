...
from extractors.rainbet_http.extractor import RainbetHttpExtractor, _tournament_id_from_url
...
class RainbetParserTests(unittest.TestCase):
    def test_build_competition_maps_1x2_and_goal_line(self) -> None:
        extraction = build_competition_extraction(
            tournament_id="100", snapshot=_snapshot(), source_url="rainbet:tournament:100"
        )
        self.assertEqual(extraction.platform, "rainbet_http")
    ...
    def test_empty_for_unknown_tournament(self) -> None:
        extraction = build_competition_extraction(
            tournament_id="999", snapshot=_snapshot(), source_url="rainbet:tournament:999"
        )
        self.assertTrue(extraction.is_empty)


class RainbetDiscoveryTests(unittest.TestCase):
    def test_build_league_options_soccer_only_by_country(self) -> None:
        options = discovery_module.build_league_options(
            ...
            platform_display_name="Rainbet HTTP",
            sport_id="1",
            country_name="brazil",
        )
        ...
        self.assertEqual(opt.source_url, "rainbet:tournament:100")
...
class RainbetUrlTests(unittest.TestCase):
    def test_tournament_id_from_scheme(self) -> None:
        self.assertEqual(_tournament_id_from_url("rainbet:tournament:1669818812230406144"), "1669818812230406144")

    def test_tournament_id_from_bt_path(self) -> None:
        url = "https://rainbet.com/sports?bt-path=%2Fsoccer%2Fbrazil%2Fbrasileiro-serie-a-1669818812230406144"
        self.assertEqual(_tournament_id_from_url(url), "1669818812230406144")

    def test_tournament_id_missing(self) -> None:
        self.assertIsNone(_tournament_id_from_url("https://rainbet.com/sports"))

    def test_can_handle_url(self) -> None:
        self.assertTrue(RainbetHttpExtractor.can_handle_url("rainbet:tournament:1669818812230406144"))
        self.assertTrue(RainbetHttpExtractor.can_handle_url("https://rainbet.com/sports?bt-path=%2Fsoccer"))
        self.assertFalse(RainbetHttpExtractor.can_handle_url("https://mystake.bet/x"))

    def test_feed_url_shape(self) -> None:
        settings = RainbetHttpSettings(api_host="h.sptpub.com", brand_id="123", language="en")
