...
class RefreshIntervalTests(unittest.TestCase):
    def test_interval_by_proximity(self) -> None:
        ...
        # in-play (started 30 min ago) -> fast
        self.assertEqual(_refresh_interval_seconds(NOW, NOW - timedelta(minutes=30)), 60.0)
        # 10 min to kickoff -> fast
        self.assertEqual(_refresh_interval_seconds(NOW, NOW + timedelta(minutes=10)), 60.0)
        # 2 h -> normal
...
class IsRefreshDueTests(unittest.TestCase):
    ...
    def test_in_play_is_due_each_cycle(self) -> None:
        ko = NOW - timedelta(minutes=20)  # in-play, 60 s tier
        self.assertTrue(_is_refresh_due(NOW, NOW - timedelta(seconds=120), ko))
