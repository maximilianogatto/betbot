...
The SQLite database is gitignored and does not travel
...
from adapters.storage.connection import open_connection, PROJECT_ROOT, resolve_database_path
from adapters.storage.competitions import _insert_unified_competition, _find_or_create_unified_competition_id
...
def _platform_dict(row) -> dict[str, Any]:
    return {
        ...
        "competition_external_id": row["external_id"],
        "competition_name": row["name"],
        "source_url": row["source_url"],
        "metadata_json": row["metadata_json"],
        "needs_name_resolution": int(row["needs_name_resolution"] if "needs_name_resolution" in row.keys() else 0),
    ...
    with open_connection() as connection:
        ...
        for uc in connection.execute(
            ...
            platforms = [
                ...
                for r in connection.execute(
                    ...
                    SELECT platform, external_id, name,
                           source_url, metadata_json, enabled, reminders_enabled
                    FROM competitions
            ...
            stats_links = [
                ...
                for r in connection.execute(
                    """
                    SELECT s.provider AS stats_provider, s.league_id AS stats_league_id, s.league_name AS stats_league_name,
                           s.country_name AS stats_country_name, s.confidence, s.payload_json
                    FROM stats_league_links s
...
def write_seed_file(path: Path | None = None) -> Path:
    """Export the current DB state and save it to the seed JSON file."""
    data = export_league_seed()
    dest = path or SEED_FILE_PATH
...
def load_seed_file(path: Path | None = None) -> dict[str, Any] | None:
    """Read the seed file if it exists, otherwise return None."""
    src = path or SEED_FILE_PATH
    if not src.exists():
        return None
...
def _flatten_v2(data: dict[str, Any]) -> dict[str, Any]:
    """Convert a v2 seed format back to flat v1 format for the importer."""
    unified_names: list[str] = []
    tracked: list[dict[str, Any]] = []
    stats_links: list[dict[str, Any]] = []

    for league in data.get("leagues") or []:
        unified_name = league.get("name")
        if not unified_name:
            continue
        unified_names.append(unified_name)

        # To attach stats links to a tracked competition (as v1 did), we must
        # anchor it to ONE of the platform tracked competitions. We pick the first one.
        platforms = league.get("platforms") or []
        for platform_entry in platforms:
            tracked.append({**platform_entry, "unified_name": unified_name})
    ...
    with open_connection() as connection:
        ...
        for tc in data.get("tracked_competitions") or []:
            ...
            existing = connection.execute(
                "SELECT id, unified_competition_id FROM competitions "
            ...
            if existing is None:
                cursor = connection.execute(
                    ...
                    INSERT INTO competitions (
                        ...
                        metadata_json, enabled, reminders_enabled,
                        unified_competition_id, consecutive_unavailable_refreshes, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)
                    """,
                    (
                        platform, ext_id,
                ...
                if overwrite:
                    connection.execute(
                        """
                        UPDATE competitions
                        SET name = ?, source_url = ?, metadata_json = ?,
        ...
        for link in data.get("stats_league_links") or []:
            ...
            if tracked_id is None:
                row = connection.execute(
                    "SELECT id FROM competitions WHERE platform = ? AND external_id = ?",
                    (platform, ext_id),
                ).fetchone()
            ...
            existing = connection.execute(
                "SELECT id FROM stats_league_links WHERE competition_id = ? AND provider = ?",
                (tracked_id, provider),
            ).fetchone()
            if existing is None:
                connection.execute(
                    ...
                    INSERT INTO stats_league_links (
                        competition_id, provider, league_id, league_name, country_name,
                ...
                connection.execute(
                    ...
                    SET league_id = ?, league_name = ?, country_name = ?,
...
def _tracked_competitions_count() -> int:
    with open_connection() as connection:
        row = connection.execute("SELECT COUNT(*) AS c FROM competitions").fetchone()
...
def seed_if_empty(path: Path | None = None) -> dict[str, int] | None:
    ...
    try:
        ...
        logger.info(
            ...
            db_path, counts["unified_created"], counts["tracked_inserted"], counts["stats_inserted"],
        )
        return counts
