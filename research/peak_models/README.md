...
- **`g1_poisson_dc.ipynb`** — análisis de EXP-001: Poisson y Dixon-Coles
  (decaimiento 120d + shrinkage + ρ) vs G0 bajo walk-forward semanal 2026.
  Resultado: DC 0.2107 < Poisson 0.2159 < G0 0.2193 < base 0.2377 (RPS).
  Detalle y bootstrap en `../experiments/EXP-001-model-ladder/report.md`.
- **`models.py`** — fit Poisson/Dixon-Coles (MLE + decaimiento + ridge) y
...
Próximos (ver `../PROTOCOLO_INVESTIGACION.md`):
1. EXP-002: diagnóstico M1 (única liga donde DC pierde vs G0) + capa logística
   de calibración sobre λs.
2. EXP-003: jerárquico bayesiano multi-liga; extender el builder a
